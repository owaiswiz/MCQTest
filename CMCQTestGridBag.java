...
class Test implements ActionListener
{
	...
	{
		String csvFile = "." + File.separator + "files" + File.separator + "questions.txt";
		String line = "";
	...
	Test(String name,String ip,String username,String password)
	{
		this.ip = ip;
		this.name = name;
	...
	public void actionPerformed(ActionEvent ae)
	{
		...
		{
			InteractWithFTPServer gc = new InteractWithFTPServer(ip,username,password);
			gc.downloadCountFile();
			try
			{
				String line = "";
				BufferedReader br = new BufferedReader(new FileReader("." + File.separator + "files" + File.separator + "count.txt"));
				while(( line = br.readLine()) != null)
				{
	...
	void displayOthersResult()
	{
		...
		int count[] = new int[1];
		InteractWithFTPServer gr = new InteractWithFTPServer(ip,username,password);
	...
	{
		InteractWithFTPServer sr = new InteractWithFTPServer(ip,username,password);
...
class InitialForm implements ActionListener
{
	...
	Font bigArialFont;
	JButton goToTest;
	JTextField nameTextField,ipTextField,usernameTextField,passwordTextField;
	...
	InitialForm()
	{
		...
		c.gridx = 1;
		c.gridy = 3;
		usernameTextField = new JTextField("pi");
		usernameTextField.setFont(new Font("Arial",Font.PLAIN,16));
		f.add(usernameTextField,c);

		c.gridx = 0;
		c.gridy = 4;
		JLabel passwordLabel = new JLabel("Enter FTP Password");
		passwordLabel.setFont(moderateArialFont);
		f.add(passwordLabel,c);
		
		c.gridx = 1;
		c.gridy = 4;
		passwordTextField = new JTextField("raspberry");
		passwordTextField.setFont(new Font("Arial",Font.PLAIN,16));
		f.add(passwordTextField,c);
	...
	public void actionPerformed(ActionEvent ae)
	{
		if(ae.getSource() == goToTest)
		{
			...
			if(name.length() == 0 || ip.length() == 0 || username.length() == 0 || password.length() == 0)
			{
			...
			else
			{
				...
				new Test(name,ip,username,password);
			}
		}
	}
	...
	{
		InteractWithFTPServer dq = new InteractWithFTPServer(ip,username,password);
...
{
	FTPClient ftp;
	String remoteDirectory,localDirectory,ip,userId,password;

	InteractWithFTPServer(String ip,String username, String password)
	{
		this.ip = ip;
	...
	void downloadFile(String file)
	{
		try
		{
			connectToFTPServer();
			OutputStream output;
			output = new FileOutputStream(localDirectory + File.separator + "files" + File.separator + file);
	...
	void uploadFile(String fileName)
	{
		try
		{
			connectToFTPServer();
			InputStream input;
			input = new FileInputStream(localDirectory + File.separator + "files" + File.separator + fileName);
	...
	void sendResult(String name,int marks)
	{
		...
		try
		{
			File file = new File(localDirectory + File.separator + "files" + File.separator + fileName);
			file.createNewFile();
			uploadFile(fileName);
	...
	void changeCountFile(String change)
	{
		...
		try {
			BufferedReader br = new BufferedReader(new FileReader(localDirectory + File.separator + "files" + File.separator + "count.txt"));
			String storedCount;
			...
			out = new BufferedWriter(new FileWriter(localDirectory + File.separator + "files" + File.separator + "count.txt", false));
